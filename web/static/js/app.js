...
function applyConfig(cfg) {
  ...
  const r = cfg.remote || {};
  setVal('r-host', r.host || '');
  setVal('r-port', r.port || '8899');
...
async function testRelay() {
  const host = getVal('r-host').trim();
  const port = parseInt(getVal('r-port')) || 8899;

  if (!host) { alert('请输入服务器地址'); return; }

  const btn = document.getElementById('r-test-btn');
  btn.textContent = '⏳ 测试中...';
  btn.disabled = true;

  try {
    const result = await API.relayTest(host, port);
    const status = document.getElementById('r-conn-status');
    if (result.ok) {
      state.remote.connected = true;
      status.textContent = '● 已连接 ' + host + ':' + port;
...
async function searchRemote() {
  ...
  try {
    ...
    state.remote.info = info;
    renderRemoteInfo(info);
    renderRemoteFormats(info.formats || []);
    renderRemoteAudio(info.audio_tracks || []);
  } catch (e) {
    alert('搜索失败: ' + e.message);
  } finally {
    btn.textContent = '🔍 搜索';
...
function renderRemoteFormats(formats) {
  const tbody = document.getElementById('r-format-body');
  tbody.innerHTML = '';
  state.remote.selectedFormat = null;

  if (!formats || formats.length === 0) {
    tbody.innerHTML = '<tr><td colspan="5" class="empty-row">⚠ 该视频暂无可用下载格式</td></tr>';
    return;
  }
  ...
  formats.forEach((f, i) => {
    ...
    tr.addEventListener('click', () => {
      tbody.querySelectorAll('tr').forEach(r => r.classList.remove('selected'));
      tr.classList.add('selected');
      state.remote.selectedFormat = f;
    });
    tbody.appendChild(tr);
    if (i === 0) { tr.classList.add('selected'); state.remote.selectedFormat = f; }
  });
  showCard('r-format-card');
}

function renderRemoteAudio(tracks) {
  const tbody = document.getElementById('r-audio-body');
  tbody.innerHTML = '';
  state.remote.selectedAudioIds = [];
  if (!tracks || tracks.length === 0) {
    document.getElementById('r-audio-card').style.display = 'none';
    return;
  }
  showCard('r-audio-card');
  ...
  tracks.forEach(t => {
    ...
    const cb = tr.querySelector('input[type="checkbox"]');
    cb.checked = true;
    state.remote.selectedAudioIds.push(t.format_id);
    cb.addEventListener('change', () => {
      if (cb.checked) {
        state.remote.selectedAudioIds.push(t.format_id);
      } else {
        state.remote.selectedAudioIds = state.remote.selectedAudioIds.filter(id => id !== t.format_id);
      }
    });
    tbody.appendChild(tr);
  });
...
async function submitRemote() {
  if (!state.remote.connected) { alert('请先连接 VPS 服务器'); return; }
  if (!state.remote.info || !state.remote.selectedFormat) { alert('请先搜索视频并选择分辨率'); return; }

  const host = getVal('r-host').trim();
  const port = parseInt(getVal('r-port')) || 8899;

  let formatId = state.remote.selectedFormat.video_id;
  if (state.remote.selectedAudioIds.length > 0) {
    formatId = formatId + '+' + state.remote.selectedAudioIds.join('+');
  }
  ...
  try {
    await API.relaySubmit(host, port, state.remote.info.url, formatId, getVal('r-fmt'));
    alert('任务已下发到 VPS');
    refreshRemote();
  } catch (e) {
    alert('下发失败: ' + e.message);
  } finally {
    btn.textContent = '📤 下发到 VPS 下载';
    btn.disabled = false;
  }
}
...
async function refreshRemote() {
  ...
  try {
    const [tasks, files] = await Promise.all([
      API.relayTasks(host, port),
      API.relayFiles(host, port),
    ]);
    renderRemoteTasks(tasks);
    renderRemoteFiles(files);
  } catch (e) {
    console.warn('刷新远程失败:', e);
  }
}

function renderRemoteTasks(tasks) {
  const tbody = document.getElementById('r-task-body');
  ...
  const statusMap = {
    pending: '等待中', downloading: '下载中', merging: '合并中',
    done: '✅ 已完成', error: '❌ 失败',
  };
  ...
  tbody.innerHTML = tasks.map(t => `
    <tr>
      <td>${t.task_id || ''}</td>
      <td>${(t.title || t.url || '').substring(0, 50)}</td>
      <td>${t.format_note || ''}</td>
      <td>${statusMap[t.status] || t.status}</td>
      <td>${t.status === 'downloading' || t.status === 'merging' ? (t.percent || 0).toFixed(1) + '%' : statusMap[t.status] || t.status}</td>
    </tr>
  `).join('');
}

function renderRemoteFiles(files) {
  const tbody = document.getElementById('r-file-body');
  ...
  tbody.innerHTML = files.map(f => `
    <tr>
      <td>${f.filename}</td>
      <td>${(f.size_mb || 0).toFixed(1)}</td>
      <td>${(f.created_at || '').substring(0, 16)}</td>
      <td>
        <button class="btn btn-primary btn-sm" onclick="downloadRemoteFile('${f.filename}')">⬇ 下载</button>
        <button class="btn btn-danger btn-sm" onclick="deleteRemoteFile('${f.filename}')">🗑</button>
      </td>
    </tr>
  `).join('');
}

async function downloadRemoteFile(filename) {
  const saveDir = getVal('l-save-dir').trim() ||
    prompt('请输入保存目录:') || '';
  if (!saveDir) { alert('请提供保存目录'); return; }

  const host = getVal('r-host').trim();
  const port = parseInt(getVal('r-port')) || 8899;

  try {
    await API.relayDownload(host, port, filename, saveDir);
    alert('下载完成: ' + filename);
  } catch (e) {
...
async function deleteRemoteFile(filename) {
  if (!confirm('确定要删除 VPS 上的\n' + filename + '\n吗？')) return;

  const host = getVal('r-host').trim();
  const port = parseInt(getVal('r-port')) || 8899;

  try {
    await API.relayDeleteFile(host, port, filename);
    refreshRemote();
  } catch (e) {
